...
int main(int argc, char* argv[])
{
	...
	//================================== Launch our kernel	==============================//
    ms = assemblySgemm("sgemm_nn_192x192", devRand, devC, devA, devB, M, N, K, hStart, hStop);
    gflops(M, K, N, ms);
    ...
    for(int i = 0; i < iterations; i ++) {
        cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_T, M, N, K, &alpha, devA, K, devB, N, &beta, devC, M);
    }
...
#if 0
    filec=fopen("cublas.txt", "w");
    for (int i = 0; i < M; i ++) {
        int index, j;
        for  (j = 0; j < N; j ++) {
            index= j*M + i;
            fprintf(filec, "%f ", cublasC[index]);
        }
        fprintf(filec, "\n");
    }
    fclose(filec);
...
float assemblySgemm(const char* kernel, float * devRand, float * devC, float * devA, float * devB, int M, int N, int K, CUevent hStart, CUevent hStop)
{
	...
	// Load the cubin
	CUmodule hModule;
	CUDA_CHECK( cuModuleLoad(&hModule, "sgemm_nn_192x192.cubin") );
    ...
    int loops = 0;
    //int k = N;
	void* params[] = {&devRand, &devA, &devB,  &devC, &lda, &ldb, &ldc, &M, &N, &K, &alpha, &beta, &flags, &ldaz, &ldbz, &ldcz, &devD};
