...
int main(int argc, char* argv[])
{
	...
	//================================== Launch our kernel	==============================//
    ms = assemblySgemm("sgemm_tt_128x128", devRand, devC, devA, devB, M, N, K, hStart, hStop);
    gflops(M, K, N, ms);
    ...
    for(int i = 0; i < iterations; i ++) {
        cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, K, &alpha, devA, K, devB, N, &beta, devC, M);
    }
...
float assemblySgemm(const char* kernel, float * devRand, float * devC, float * devA, float * devB, int M, int N, int K, CUevent hStart, CUevent hStop)
{
	...
	// Load the cubin
	CUmodule hModule;
	CUDA_CHECK( cuModuleLoad(&hModule, "sgemm_tt_128x128.cubin") );
